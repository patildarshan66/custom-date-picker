...
import 'package:custom_date_picker/customDatePicker/screens/six_preset_header.dart';
import 'package:custom_date_picker/utils/extensions.dart';
...
import 'four_preset_header.dart';
...
Future<DateTime?> showCustomDatePicker({
  ...
  DatePickersType datePickersType = DatePickersType.noPreset,
}) async {
  ...
  Widget dialog = CustomDatePickerDialog(
    ...
    datePickersType: datePickersType,
  );
...
class CustomDatePickerDialog extends StatefulWidget {
  ...
  CustomDatePickerDialog(
      ...
      this.fieldLabelText,
      this.restorationId,
      this.datePickersType = DatePickersType.noPreset})
      : initialDate = DateUtils.dateOnly(initialDate),
        firstDate = DateUtils.dateOnly(firstDate),
        lastDate = DateUtils.dateOnly(lastDate),
  ...
  final DatePickersType datePickersType;
...
class _CustomDatePickerDialogState extends State<CustomDatePickerDialog>
  ...
  Widget build(BuildContext context) {
        ...
        children: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
                elevation: 0, primary: lightSkyBlueColor),
            onPressed: _handleCancel,
            child: Text(
              widget.cancelText ?? localizations.cancelButtonLabel,
              style: fontLFontL1(color: primaryColor),
            ),
          ),
          ...
          ElevatedButton(
            ...
            child: Text(widget.confirmText ?? localizations.okButtonLabel,
                style: fontLFontL1(color: Colors.white)),
          ),
        ],
      ),
    );
    ...
    final Widget header = widget.datePickersType == DatePickersType.preset4
        ? DatePickerFourPresetHeader(
            orientation: orientation,
            onTap: (selectedPreset) {
              switch (selectedPreset) {
                case FourPresets.days15:
                  _selectedDate.value =
                      DateTime.now().add(const Duration(days: 15));
                  break;
                case FourPresets.days30:
                  _selectedDate.value =
                      DateTime.now().add(const Duration(days: 30));
                  break;
                case FourPresets.days60:
                  _selectedDate.value =
                      DateTime.now().add(const Duration(days: 60));
                  break;
                default:

                  /// ever ends value ( I have set last calender date to ever ends value) because calender not allows me to assign date after the last date.
                  _selectedDate.value = widget.lastDate;
                  break;
              }
              setState(() {});
            })
        : DatePickerSixPresetHeader(
            orientation: orientation,
            onTap: (selectedPreset) {
              switch (selectedPreset) {
                case SixPresets.yesterday:
                  _selectedDate.value =
                      DateTime.now().add(const Duration(days: -1));
                  break;
                case SixPresets.today:
                  _selectedDate.value = DateTime.now();
                  break;
                case SixPresets.tomorrow:
                  _selectedDate.value =
                      DateTime.now().add(const Duration(days: 1));
                  break;
                case SixPresets.thisSaturday:
                  _selectedDate.value = DateTime.now().next(DateTime.saturday);
                  break;
                case SixPresets.thisSunday:
                  _selectedDate.value = DateTime.now().next(DateTime.sunday);
                  break;
                case SixPresets.nextTuesday:
                default:
                  final thisTuesday = DateTime.now().next(DateTime.tuesday);
                  _selectedDate.value = thisTuesday.next(DateTime.tuesday);
                  break;
              }
              setState(() {});
            });
            ...
            switch (orientation) {
              case Orientation.portrait:
                return Column(
                  ...
                  children: <Widget>[
                    if (widget.datePickersType != DatePickersType.noPreset)
                      header,
                    picker,
                    actions,
                  ],
